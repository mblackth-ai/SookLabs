  ...
  for (const [streamKey, stream] of Object.entries(ops.workstreams || {})) {
    for (const item of stream.items || []) {
      ...
      items.push({
        ...
        boardHref: `${stream.boardHref || "/hq/sookly/action-plan"}?focus=${encodeURIComponent(item.id)}`,
      });
    }
  }
  items.sort((a, b) => {
...
/** Open + P0 counts for all workstreams sharing a board href (e.g. Sookly website + app). */
export function getBoardOpenCounts(ops, boardHref) {
  const href = boardHref || ops.primaryBoardHref || "/hq/sookly/action-plan";
  const items = Object.values(ops.workstreams || {})
    .filter((s) => (s.boardHref || "/hq/sookly/action-plan") === href)
    .flatMap((s) => s.items || []);
  const open = items.filter((i) => i.status !== "done");
  return {
    open: open.length,
    ...
    for (const item of stream.items || []) {
      ...
      if (item.status === "blocked") {
        attention.push({
          ...
          tag: stream.label,
          action: "Unblock",
          href: `${stream.boardHref || "/hq"}?focus=${encodeURIComponent(item.id)}`,
          id: item.id,
        });
  ...
  for (const [streamKey, stream] of Object.entries(normalized.workstreams || {})) {
    for (const item of stream.items || []) {
      ...
      fromStreams.push({
        ...
        href: `${stream.boardHref || "/hq/portfolio"}?focus=${encodeURIComponent(item.id)}`,
      });
    }
  }
