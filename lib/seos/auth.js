// Phase 1 SEOS auth: single shared password -> HMAC-signed session token.
// Edge-safe (Web Crypto only). Separate cookie/env from HQ.

export const SEOS_COOKIE = "seos_session";
...
export function resolveSessionSecret(raw = process.env.SEOS_SESSION_SECRET) {
...
export function resolveAccessPassword(raw = process.env.SEOS_ACCESS_PASSWORD) {
...
export async function verifyPassword(submitted, password) {
  ...
  const [submittedHash, passwordHash] = await Promise.all([sha256Hex(submitted), sha256Hex(password)]);
