...
export function EndOfDayCard({ initialData }) {
  ...
  return (
    <Card padding="md" className="hq-eod-card">
      <div className="hq-eod-header">
        <div>
          <span className="hq-eod-title">End of day</span>
          <div className="hq-eod-subtitle">~2 min — clear done, carry one, reset loop</div>
        </div>
      ...
      {!decidedToday ? (
        <p className="hq-eod-nudge">
          No decision logged today — optional, but useful when you chose a path.{" "}
          <a href="/hq/decision-log">Log one →</a>
        </p>
      ) : null}
      {confirmClose ? (
        <div className="hq-eod-confirm" role="status">
          <p className="hq-eod-confirm-text">
            Close day? Keeps your first unfinished priority, clears done items, and resets the morning loop.
          </p>
          <div className="hq-eod-actions">
            <Button variant="accent" size="sm" disabled={saving} onClick={closeDay}>
              Yes, close day
            </Button>
            <Button variant="ghost" size="sm" disabled={saving} onClick={() => setConfirmClose(false)}>
              Cancel
            </Button>
          </div>
        </div>
        ...
        <div className="hq-eod-actions">
          ...
          <Button variant="ghost" size="sm" disabled={saving} onClick={() => setConfirmClose(true)}>
            Close day…
          </Button>
